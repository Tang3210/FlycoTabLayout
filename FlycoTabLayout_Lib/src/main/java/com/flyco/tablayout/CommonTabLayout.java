...
public class CommonTabLayout extends FrameLayout implements ValueAnimator.AnimatorUpdateListener {
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mTextSelectSize = ta.getDimension(R.styleable.CommonTabLayout_tl_textSelectSize, -1);
...
//        Log.d("AAA", "mCurrentP--->" + mCurrentP.left + "&" + mCurrentP.right);
    ...
    public void setMsgMargin(int position, float leftPadding, float bottomPadding) {
        ...
        if (tipView != null) {
            ...
            if (mIconVisible) {
                if (iconH <= 0) {
                    Drawable drawable = ContextCompat.getDrawable(mContext, mTabEntitys.get(position).getTabSelectedIcon());
                    if (drawable != null) {
                        iconH = drawable.getIntrinsicHeight();
                    }
                }
                margin = mIconMargin;
            }
